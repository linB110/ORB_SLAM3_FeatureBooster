...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, 
                ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF,
                const IMU::Calib &ImuCalib, FeatureBooster* pFeatureBooster)
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    // FeatureBooster
    if (pFeatureBooster){
        std::thread t1 ([&](){pFeatureBooster->BoostDescriptors(mvKeys, mDescriptors);});
        std::thread t2 ([&](){pFeatureBooster->BoostDescriptors(mvKeysRight, mDescriptorsRight);});
        t1.join();
        t2.join();
    }
...
// Monocular
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib, FeatureBooster *pFeatureBooster)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
